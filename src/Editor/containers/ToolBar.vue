...
<style scoped lang="less" rel="stylesheet/less">
  .tool-bar {
    ...
    background-image:url(../../assets/images/media_top.png);
  ...
  .preview-hover:hover .pre-icon{// yue
    color:#ccc;
  }
</style>
            ...
            <template v-slot:label>
              ...
              <template v-else>
                <XColorPicker v-model="formData[item.name]" @on-change="(val) => handleToolClick(item, val, null)">
                  <div style="margin: 0 3px;color:#fff" slot="preview" class="preview-hover"><!--yue-->
                    <XIcon :iconfont="item.icon" :label="handleLabel(item)" style="vertical-align: middle;" class="pre-icon"></XIcon>
                    <Icon type="ios-arrow-down" class="pre-icon"></Icon>
                  </div>
                </XColorPicker>
              </template>
            </template>
