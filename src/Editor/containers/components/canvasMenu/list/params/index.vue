          ...
          <div class="form-container">
            <div class="form-left" style="margin-top:-25px">
              <div class="relation-form">
                <FormItem label="能源绩效参数" prop="paramsName" :label-width="110">
                    <Input v-model="formValidate.paramsName" placeholder="请输入能源绩效参数" clearable :disabled="editOrAddFlag"></Input>
                </FormItem>
                <!--关联因子-->
                <FormItem
              ...
              <div class="relation-table" @mouseenter="suspensionEnterOr(2)" @mouseleave="suspensionEnterOr(1)">
                  <Table :columns="columnsRelation" highlight-row height="275" stripe border :data="dataRelation" @on-current-change="handleObjectRowChange"></Table>
                  <div class="suspension-table">
                      <Table :columns="columnsRelationAttr" v-show="attrTableShow" height="275" stripe border :data="dataRelationAttr">
                         <template slot-scope="{ row, index }" slot="action">
                          <Button type="primary" @click="copyTemplate(index, row)" size="small">复制</Button>
                        </template>
                      </Table>
                  </div>
              </div>
            ...
            <div class="form-right">
              <div class="form-introduce">
                <FormItem prop="CFIntroduce" :label-width="0">
                    <div><span style="color:red;margin-right:4px;font-size:15px" v-html="'*'"/>计算公式说明</div>
                    <Input v-model="formValidate.CFIntroduce" type="textarea" :autosize="{minRows: 4,maxRows: 4}" placeholder="请输入计算公式说明"></Input>
                </FormItem>
              </div>
...
export default {
  ...
  data() {
    return {
      ...
      ruleValidate: {
        ...
        CFIntroduce: [
          { required: true, message: "计算公式说明不能为空", trigger: "blur" }
        ],
        correlationFactorId: [
          { required: false, message: "关联因子不能为空", trigger: "blur" }
        ]
      },
      columns: [
        {
  ...
  computed: {
    dataRelationAttrDo(){
      return this.dataRelationAttr;
    }
  },
  methods: {
    // 复制模板内容
    copyTemplate(index, data) {
      console.log(data)
      this.copyToClipboard(`${ data.name } - ${ data.attrId }`); // 需要复制的文本内容
      this.$Message.success('复制成功：可 “Ctrl + V” 进行粘贴');
    ...
    // 点击对象属性触发的方法
    handleObjectRowChange(e){
      console.log(e)
      this.dataRelationAttr = this.dataRelationAttr.map(_ => {
        _.attrId = `${ e.objectId * Math.random() << 0 } - ${ new Date().toLocaleString('chinese', { hour12: false }).split(" ")[1] }`
        return _;
      })
      console.log(this.dataRelationAttr)
    },
    // 属性表格显隐控制
    suspensionEnterOr(e){
      e == 2 ? this.attrTableShow = true : this.attrTableShow = false;
    },
