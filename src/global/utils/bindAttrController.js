 ...
 */
export const updateBA = (graph, nodeId, attrIds = [], type) => {
    if(!graph || !nodeId) throw new Error('graph or nodeId must be requested! 蓝之静云')
    ...
    if(Object.prototype.toString.call(type) === '[object Boolean]'){
        originBindAttrs.map((_, _index) => {
            if(attrIds.includes(_._attrs.data.id)) originBindAttrs[_index]._cfg.visible = type;
        })
    }else{
        switch(type){
            case 'position':// 更新位置
