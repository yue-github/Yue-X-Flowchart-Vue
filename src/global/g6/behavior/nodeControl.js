...
export default {
  ...
  options: {
    ...
    onNodeMousedown (event) {
      ...
      const { x, y } = model;
     // -
     _t.disX = event.clientX - x;// 2020-7-25 by WuTongYue
     _t.disY = event.clientY - y;
     _t.isLeave = false;
    ...
    onCanvasMouseleave (event) {
      ...
      const _t = this
      if (_t.info && _t.info.type === 'dragNode' && _t.isLeave) {
        _t[_t.info.type].stop.call(_t, event)
      }
    },
          ...
          if (_t.info.node) {
            ...
            _t.graph.find('node', node => {
              ...
              if (model.id === id) {
               attrs  = {
                  // 处理点击移动 图形时的抖动
                  x: event.clientX - _t.disX,
                  y: event.clientY - _t.disY
                }
                // 更新节点
                _t.graph.updateItem(_t.info.node, attrs)
